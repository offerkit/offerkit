...
```bash
OFFERKIT_IMAGE_TAG=v1.2.3 docker compose up -d
```
...
For production, pin both image services to a version tag such as `ghcr.io/offerkit/offerkit:v1.2.3` instead of `latest`.
