...
With `OFFERKIT_PUBLIC_URL` unset, OfferKit derives the exact origin of each authentication request. Open the server's LAN address directly—for example, `http://192.168.1.108:3000`. No origin configuration is needed for direct localhost or LAN access, and cross-site origins remain blocked.

Set `OFFERKIT_PUBLIC_URL` for production deployments, reverse proxies, OAuth clients, or hosted MCP. If a configured deployment should accept authentication from additional browser origins, list their exact origins in the comma-separated `BETTER_AUTH_TRUSTED_ORIGINS` variable. Do not use a broad wildcard or disable Better Auth's origin check.
